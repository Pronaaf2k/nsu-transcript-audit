...
# ── Path setup ────────────────────────────────────────────────────────────────
# audit_src/ must sit alongside this file and contain:
#   audit_l1.py, audit_l2.py, audit_l3.py, style.py, program.md
AUDIT_SRC = Path(__file__).parent / "audit_src"
...
def _run_audit(csv_text: str, program: str) -> dict[str, Any]:
    ...
    try:
        # Force reload so modules always use current sys.path
        for mod_name in ["audit_l1", "audit_l2", "audit_l3", "style"]:
