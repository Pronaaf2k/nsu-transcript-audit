...
def _audit_csv_file(args: tuple[str, str, str]) -> dict[str, Any]:
    ...
    import sys
    sys.path.insert(0, str(Path(__file__).parent / "audit_src"))

    from audit_engine_local import run_audit_local  # type: ignore[import]
    try:
        result = run_audit_local(csv_path, program)
...
def _audit_image_file(args: tuple[str, str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False, encoding="utf-8") as f:
            ...
            tmp = f.name
        try:
            result = run_audit_local(tmp, program)
            return {"file": file_name, "status": "ok", "result": result}
        finally:
            os.unlink(tmp)
    except Exception as e:
