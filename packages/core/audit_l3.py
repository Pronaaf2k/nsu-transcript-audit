...
def parse_global_courses(md_file):
    """
    Scans ALL programs in program.md and returns a dict:
      { course_code: credits }
    for every course listed in any program's requirement sections.
    Used to distinguish valid NSU free electives from truly invalid courses.
    """
    global_registry = {}
        ...
        with open(md_file, 'r') as f:
            for line in f:
                ...
                if line.startswith('- **Prerequisites**:'):
                    in_req_section = False   # stop before prereq section
                    continue
...
def parse_program_knowledge(md_file, program_name):
    ...
    Returns a dict with:
    - total_credits_required, min_cgpa
    ...
    - elective_cap  (int: total_credits_required - sum of all required course credits)
    """
    requirements = {
    ...
    fixed_credits = 0   # sum of credits of all required courses
...
def audit_student(transcript_file, requirements, md_file='program.md'):
    ...
    # Build semester → set of course codes index (used for co-registration check below)
    ...
    for r in rows:
        ...
        # Grade 'T' = Transfer/Admission-test waiver: counts as passed (satisfies
        # the requirement) but is excluded from GPA calculation entirely.
        if grade.upper() == 'T':
        ...
        if grade.upper() != 'T' and GRADE_POINTS.get(grade) is not None:
            ...
            if existing is not None and existing['points'] >= 3.3:
                # Student already holds B+ or higher — retake not permitted
                advisories.append(
    ...
    # ── 0-credit lab co-registration check ───────────────────────────────────
    # Rule: 0-credit labs must be enrolled in the SAME semester as the main
    # lecture. 1-credit labs (PHY107L, etc.) may be taken/retaken independently.
    ...
    seen_lab_violations = set()  # avoid duplicate advisories for multiple retakes
    for r in rows:
        course  = r['course']
        credits = r['credits']
        sem     = r['semester']
        # Only CSE 0-credit lab courses — PHY/CHE/BIO labs are fully independent
        if credits == 0 and course.startswith('CSE') and course.endswith('L'):
            main = course[:-1]   # e.g. CSE225L → CSE225
    ...
    # Equivalent course substitutions (e.g. BUS112 <-> MAT112 for some programs)
    ...
    # Global registry: every NSU course code from any program in program.md.
    # A course not in this registry is not an NSU course → invalid elective.
    global_registry = parse_global_courses(md_file)

    elective_cap        = requirements.get('elective_cap', 0)
    free_elective_used  = 0.0
    free_electives      = []    # (course, credits) — valid NSU free electives
    excess_electives    = []    # (course, credits) — over cap (non-blocking advisory)
    invalid_electives   = []    # course codes not in ANY NSU program
    ...
    # Elective classification — chronological order (first passed comes first)
    seen_order = []
    for r in rows:
        c = r['course']
    ...
    for course in seen_order:
        ...
        if course in global_registry or course in equivalent_courses:
            # Valid NSU free elective — ALL count toward credit total
            total_earned_credits += creds
            free_elective_used   += creds
            free_electives.append((course, creds))
            if free_elective_used > elective_cap:
                # Retroactively mark this course as excess (advisory only, still counts)
                excess_electives.append((course, creds))
        else:
            # Not a recognised NSU course code → excluded from credit count
...
def print_report(audit, requirements, program_name):
    ...
    # excess_electives fire advisory only — they still count toward credits
    ...
    # ── Free elective summary (always shown when eligible) ───────────────────
...
def main():
    ...
    # 2. Audit (pass md_file so global registry uses same file)
